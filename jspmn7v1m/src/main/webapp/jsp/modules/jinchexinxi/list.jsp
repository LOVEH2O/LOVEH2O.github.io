...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">进车信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>进车信息管理</span></li>
								<li class="breadcrumb-item active"><span>进车信息列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">进车信息列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<label>进货时间</label>
									<input id="jinhuoshijian_start" class="Wdate form-control form-control-sm" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd'})">
									至
									<input id="jinhuoshijian_end" class="Wdate form-control form-control-sm" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd'})">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('jinhuojia')">进货价<i id="jinhuojiaIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('shuliang')">数量<i id="shuliangIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('jinhuojine')">进货金额<i id="jinhuojineIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			if($('#jinhuoshijian_start').val() != null && $('#jinhuoshijian_start').val() != ''){
				searchForm.jinhuoshijian_start = $('#jinhuoshijian_start').val();
			}
			if($('#jinhuoshijian_end').val() != null && $('#jinhuoshijian_end').val() != ''){
				searchForm.jinhuoshijian_end = $('#jinhuoshijian_end').val();
			}
			getDataList();
		}		
		// 获取数据列表
        function getDataList() {
			http("jinchexinxi/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
				...
				jinhuoshijian_start : searchForm.jinhuoshijian_start,
				jinhuoshijian_end : searchForm.jinhuoshijian_end,
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var jinhuojiaCell  = document.createElement('td');

						 jinhuojiaCell.innerHTML = item.jinhuojia;

			row.appendChild(jinhuojiaCell);
			var shuliangCell  = document.createElement('td');

						 shuliangCell.innerHTML = item.shuliang;

			row.appendChild(shuliangCell);
			var jinhuojineCell  = document.createElement('td');

						 jinhuojineCell.innerHTML = item.jinhuojine;

			row.appendChild(jinhuojineCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("jinchexinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"jinchexinxi");
